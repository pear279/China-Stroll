...
静态页面和 Pages Functions 同域接口已经部署到 Cloudflare Pages。线上 `/health` 返回 200，未登录请求和无效令牌请求返回 401，说明路由、Pages Secret 和 Supabase 会话检查均已生效。真实邮件登录与线上写入仍需使用测试账号完成一次端到端检查。
