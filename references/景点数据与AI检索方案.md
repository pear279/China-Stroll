...
7. 已确定使用硅基流动的 `BAAI/bge-m3` 作为多语言嵌入模型。Worker 已配置模型标识和 1024 维响应校验。完成检索接口与资料生成测试后再生成向量。
...
北京喜剧院坐标已经补齐。故宫、景山和天坛已按当前 OSM 底图复核为 WGS84 景区中心点，数据库记录了地图对象编号和复核时间。原坐标与景区中心分别偏移约 554 米、565 米和 408 米。其余 49 个地点保留未审核状态，进入前端前必须逐个检查。52 个景点都有来源记录，数据库现有 54 条来源。已生成 165 条检索文档，其中 126 条来自已审核内容。向量尚未生成。
...
3. 前端使用的地点必须有 WGS84 展示坐标、地图对象编号和复核时间，并在 OSM 底图上检查景区边界。入口坐标单独记录，不能代替展示中心点。
