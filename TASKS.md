...
- [ ] Task 6 — Complete Attractions 1.0
  ...
  - Progress 2026-08-31 recommendation endpoint: `POST /v1/place-recommendations` now validates the bounded preference/location/time/candidate scope, rejects unavailable candidate IDs, and ranks only published locale-matched places through the shared deterministic ranking rules. This enables account-mode recommendations immediately; optional AI explanation copy remains a later enhancement and cannot alter the deterministic order or constraints. Fresh `typecheck`, `lint`, and Worker dry run passed in the Attractions 1.0 worktree.
